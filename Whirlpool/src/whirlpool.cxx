...
namespace Whirlpool {
    ...
    Digest::Digest (const uint64_t *digest) {
        for (int i = 0 ; i < (sizeof (digest_) / 8) ; ++i) {
    ...
    size_t      Digest::Hash () const {
        ...
        for (size_t i = 0 ; i < sizeof (digest_) ; i += 4) {
            unsigned int        tmp = ((static_cast<unsigned int> (digest_ [i + 0]) << 24) |
                                       (static_cast<unsigned int> (digest_ [i + 1]) << 16) |
                                       (static_cast<unsigned int> (digest_ [i + 2]) <<  8) |
                                       (static_cast<unsigned int> (digest_ [i + 3]) <<  0)) ;
            result ^= tmp ;
        }
    ...
    int Digest::Compare (const Digest &a, const Digest &b) {
    ...
    Generator & Generator::Update (unsigned char value) {
        ...
        unsigned char * q = &buffer_ [sizeof (buffer_) - remain_] ;
        *q++ = value ;
        --remain_ ;
        AddBitCount (8) ;
        return *this ;
    ...
    void        Generator::Flush () {
        uint64_t        K0 = digest_ [0] ;
        uint64_t        K1 = digest_ [1] ;
        uint64_t        K2 = digest_ [2] ;
        uint64_t        K3 = digest_ [3] ;
        uint64_t        K4 = digest_ [4] ;
        uint64_t        K5 = digest_ [5] ;
        uint64_t        K6 = digest_ [6] ;
        uint64_t        K7 = digest_ [7] ;

        uint64_t        B0 = ToUInt64 (&buffer_ [8 * 0]) ;
        uint64_t        B1 = ToUInt64 (&buffer_ [8 * 1]) ;
        uint64_t        B2 = ToUInt64 (&buffer_ [8 * 2]) ;
        uint64_t        B3 = ToUInt64 (&buffer_ [8 * 3]) ;
        uint64_t        B4 = ToUInt64 (&buffer_ [8 * 4]) ;
        uint64_t        B5 = ToUInt64 (&buffer_ [8 * 5]) ;
        uint64_t        B6 = ToUInt64 (&buffer_ [8 * 6]) ;
        uint64_t        B7 = ToUInt64 (&buffer_ [8 * 7]) ;
    ...
    void        Generator::EmbedBitCount () {
        ...
        for (int i = ARRAY_SIZE (bitCount_) - 1 ; 0 <= i ; --i) {
            uint64_t    v = bitCount_ [i] ;
